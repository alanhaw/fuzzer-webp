...
//! The example harness was originally built for libpng, now for webp-pixbuf-loader.
...
use libafl::{
    bolts::{
        current_nanos,
        rands::{StdRand, Rand},
        tuples::{tuple_list, tuple_list_type, Named, Merge},
        AsSlice,
    },
    ...
    mutators::scheduled::{havoc_mutations, tokens_mutations, StdScheduledMutator, BitFlipMutator},
...
// NoOpMutator is only for use in debugging the fuzzer.
#[derive(Default, Debug)]
pub struct NoOpMutator;

impl<I, S> Mutator<I, S> for NoOpMutator
    where
        I: Input + HasBytesVec,
        S: HasRand,
{
    fn mutate(
        ...
        if input.bytes().is_empty() {
            ...
            //*byte ^= bit;
            // NoOp - leave the input unchanged.
...
pub type NoOpMutationsType = tuple_list_type!(
    NoOpMutator,
    );

#[must_use]
pub fn noop_mutations() -> NoOpMutationsType {
    tuple_list!(
        NoOpMutator::new(),
    )
}
...
/*
impl core::fmt::Debug for SimpleEventManager<I, MT, S>
where
    I: Input,
    MT: Monitor + Debug,
{
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "SimpleEventManager");
    }
}
*/
/// The actual fuzzer
...
*/
    ...
    // comment out in debugging fuzzer. AGH
    ...
    // include in debugging fuzzer. let mut restarting_mgr = SimpleEventManager::new(monitor);
    ...
    // Create a WEBP dictionary if not existing.
    if state.metadata().get::<Tokens>().is_none() {
        state.add_metadata(Tokens::from([
            vec![0x52, 0x49, 0x46, 0x46], // RIFF header
            "WEBP".as_bytes().to_vec(),
            "VP8X".as_bytes().to_vec(),
            "ANIM".as_bytes().to_vec(),
            "ANMF".as_bytes().to_vec(),
            "VP8L".as_bytes().to_vec(),
        ]));
    }
    ...
    // let mutator = StdScheduledMutator::new(noop_mutations());
    ...
    restarting_mgr.on_restart(&mut state)?;     // AGH comment out.
